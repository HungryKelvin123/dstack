...
- When independent work passes [`../david-mode/references/agent-runtime.md`](../david-mode/references/agent-runtime.md), put its shared recipe, verification contract, and write fences in one named file. The parent owns that file and dispatch; workers read it without changing the contract. Create a reusable skill only when the recipe will outlive this run.
- Applying this principle produces a reusable artifact: a codemod, script, generator, or bounded worker recipe. A chat claim alone does not satisfy it.
