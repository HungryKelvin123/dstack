...
Before any delegation, read [`../../../models.json`](../../../models.json). It is the single source for the active client's worker tiers, model/effort settings, concurrency, and revision limits. Every delegated role selects one of that client's `workers` entries, including searchers, implementers, reviewers, judges, and work requested through another skill or external agent tool. A role changes the brief and tier justification, never the model policy.

The user's selected high-capability parent, such as Sol, Opus, or a client-supported Fable alias, owns planning and orchestration. For broad tasks, use the highest reasoning effort that model and the active client support. Honor an explicit user effort choice. A skill cannot switch its running model or effort by declaring it: use a supported per-task control when available, otherwise keep the current settings, report the limitation once, and never claim they changed. Treat `preferredByClient` as advisory; never assume an alias exists or rewrite global client settings.
...
Client-native workers handle only bounded work with a concrete expected result. Use the narrow tier for file searches, caller inventories, data tagging, log reduction, quick summaries, formatting, mechanical edits, fixtures, and deterministic tests. Use the complex tier for an independently bounded implementation inside a settled contract, debugging with multiple hypotheses, or multi-step analysis whose result the parent can review. On Codex, both classes use Luna at max reasoning effort. On Claude Code, narrow work uses native Haiku and complex work uses native Sonnet at high effort. The parent supplies the design and acceptance checks, reviews each result, and gives specific corrections. A worker returns an unresolved design decision to the parent instead of expanding its assignment.

## Resolve the exact native worker route

Use the client's advertised model and effort controls separately. On Codex, pass `workers.codex.model` and `workers.codex.reasoningEffort` explicitly to `spawn_agent`; use a fresh or small context fork that permits overrides. A full-history fork that forces the parent model is not a valid worker route. Include the bounded task evidence in the brief instead.

On Claude Code, classify the brief before dispatch. Pass `workers.claude.narrow.model` to its native Agent/subagent route for narrow work; omit an explicit effort override unless the client advertises one for Haiku. Pass `workers.claude.complex.model` and `workers.claude.complex.effort` for complex work. Use the client's supported alias or full model ID only when it resolves to the configured route. Do not invent a bridge, install a provider, or launch an external CLI merely to work around missing model controls. Other clients need an equivalent native route or use the parent fallback.

Check advertised capability before dispatch and record the requested tier, route, and served identity when the client exposes them. If the selected tier is unavailable, the route inherits an uncontrolled model, or execution reports a different route, use the active client's `fallback` and continue in the parent. Never silently replace the worker model or lower its requested effort. Unknown served identity stays labeled unverified; it cannot prove exact execution or model diversity. Stop a mismatched worker through supported controls and reconcile any partial writes before taking over.
