...
Before any delegation, read [`../../../models.json`](../../../models.json). It is the single source for the active client's worker model, effort, concurrency, and revision limits. Every delegated role uses that client's exact `workers` entry, including searchers, implementers, reviewers, judges, and work requested through another skill or external agent tool. A role changes the brief, never the model policy.
...
Client-native workers handle bounded searches, caller inventories, log reduction, repetitive edits, routine modules, fixtures, and tests with a concrete expected result. On Codex this is Luna at max reasoning effort; on Claude Code it is the native Haiku route at the configured effort when that pair is supported. The parent supplies the relevant design and acceptance checks, reviews each result, and gives specific corrections. A worker returns an unresolved design decision to the parent instead of expanding its assignment.
...
On Claude Code, pass `workers.claude.model` and `workers.claude.effort` to its native Agent/subagent route. `haiku` is a Claude-native worker policy, not a Luna substitute. Use the client's supported alias or full model ID only when it resolves to the configured route. Do not invent a bridge, install a provider, or launch an external CLI merely to work around missing model controls. Other clients need an equivalent native route or use the parent fallback.

Check advertised capability before dispatch and record the requested route and served identity when the client exposes it. If either setting is unavailable, the route inherits an uncontrolled model, or execution reports a different route, use the active client's `fallback` and continue in the parent. Never silently replace the worker model or lower its effort. Unknown served identity stays labeled unverified; it cannot prove exact execution or model diversity. Stop a mismatched worker through supported controls and reconcile any partial writes before taking over.
