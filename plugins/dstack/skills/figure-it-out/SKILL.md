...
Open a todolist whose first item is to read the Principles section of the **david-mode** skill. Then add the phases below as todos.
...
- Judge worker artifacts in the parent before accepting them. Add an independent reviewer only for a concrete review risk under the runtime contract. If a worker fails the gate, give a bounded correction or take over. If the gate itself is wrong, fix it in its own change rather than routing around it.
