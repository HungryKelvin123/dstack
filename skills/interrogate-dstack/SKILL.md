...
Launch all reviewers in a single message using the supported subagent tools. Use the `interrogate reviewers` list from the installed dstack model profiles when present, one reviewer per entry, extending or shrinking the Reviewer A/B/C/D labels below to the configured entry count; otherwise use the table defaults.
