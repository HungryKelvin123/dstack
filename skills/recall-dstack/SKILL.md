...
4. Sweep the shared record whenever the topic names a feature, file, subsystem, area, or bug. Hand it to the **why-dstack** skill's source investigators, steering the question toward current state, failed attempts, and remaining user reports. Run available read-only sources in parallel with task-history mining. Null results are findings. Name unavailable connectors. Skip only for pure activity recall with no named target.
...
An adjacent feature or ticket stays out unless it blocks this one. When the capsule and thread lines outgrow a screen, cut detail before you cut threads. Write the brief through the **unslop-dstack** skill, cite chat findings by UUID and shared-record findings by their source (PR #, ticket ID, chat permalink, error-tracker issue), and sanitize private context before any public output.
