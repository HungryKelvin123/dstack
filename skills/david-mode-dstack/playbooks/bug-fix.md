...
4. Add a regression test when `$tdd-dstack` finds a cheap deterministic target.
5. Make the smallest root-cause fix. Avoid guards that only hide the symptom.
6. Run the focused proof, relevant suite, and repository assembly check. Review the diff with `$blast-radius-dstack` when the change crosses a boundary.
